...
<div class="flex justify-center container mt-20">
      ...
      <div class="flex flex-col">
         ...
         <h2 class="text-2xl py-2 font-semibold">{{ Auth::user()->name }}</h2>
         <h4 class="text-xl py-2">{{ Auth::user()->email }}</h4>
            <button type="button" class="text-gray-900 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-semibold rounded-full text-sm px-3 py-2.5 mr-2 my-3 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700">
               ...
               <div class="hover:translate-x-1 w-full ease-out transition-transform">
                  แก้ไขโปรไฟล์
                  <svg class="w-3.5 h-3.5 ml-2 mb-1 inline" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 10">
      ...
      <div class="mt-6 mx-16 space-y-6">
            ...
            <table class="table-fixed">
               <tr>
                  <td class="p-6 py-3 max-w-sm">คำนำหน้า:     {{ $user->name_title }}</td>
                  <td class="px-6 py-3 max-w-sm">ชื่อ:      {{ $user->name }}</td>
                  <td class="px-6 py-3 max-w-sm">นามสกุล:       {{$user->surname }}</td>
               </tr>
               <tr>
                  <td class="px-6 py-3 max-w-sm">ชื่อเล่น:     {{ $user->nickname }}</td>
                  <td class="px-6 py-3 max-w-sm">วันเกิด:      {{ $user->date_of_birth }}</td>
               </tr>
               <tr>
                  <td class="px-6 py-3 max-w-sm">รหัสนิสิต:     {{ $user->student_code }}</td>
                  <td class="px-6 py-3 max-w-sm">ชั้นปี:     {{ $user->year }}</td>
               </tr>
               <tr>
                  <td class="px-6 py-3  max-w-sm">คณะ:     {{ $user->faculty }}</td>
                  <td class="px-6 py-3 max-w-sm">สาขา:     {{$user->department }}</td>
               </tr>
               <tr>
                  <td class="px-6 py-3 max-w-sm">โรคประจำตัว:     {{ $user->congenital_disease }}</td>
                  <td class="px-6 py-3 max-w-sm">อาหารที่แพ้:      {{ $user->allergy }}</td>
               </tr>
               <tr>
                  <td class="px-6 py-6  max-w-sm">
                     <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 512 512" class="inline pr-3"><!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path d="M164.9 24.6c-7.7-18.6-28-28.5-47.4-23.2l-88 24C12.1 30.2 0 46 0 64C0 311.4 200.6 512 448 512c18 0 33.8-12.1 38.6-29.5l24-88c5.3-19.4-4.6-39.7-23.2-47.4l-96-40c-16.3-6.8-35.2-2.1-46.3 11.6L304.7 368C234.3 334.7 177.3 277.7 144 207.3L193.3 167c13.7-11.2 18.4-30 11.6-46.3l-40-96z"/></svg></i>
                     {{ $user->phone }}
                  </td>
                  <td class="px-6 py-6  max-w-sm">
